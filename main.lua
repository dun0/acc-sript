...
local BOSS_COOLDOWN_SECONDS = 6 * 60 * 60
...
local bossOrder = {308, 381, 330, 355, 458, 348, 322, 300, 366, 343}
...
local function loadBossConfig()
    ...
    for id, name in pairs(bossData) do
        defaultConfig[tostring(id)] = {
            name = name,
            difficulties = {}
        }
        for _, diff in ipairs(bossDifficulties) do
            defaultConfig[tostring(id)].difficulties[diff] = {
                enabled = false,
                teamSlot = 1,
                cooldownEnd = 0
            }
            ...
            if decodedSuccess then
                for id, bossInfo in pairs(defaultConfig) do
                    if loadedConfig[id] then
                        for diff, diffInfo in pairs(bossInfo.difficulties) do
...
local function makeDraggable(titleElement, frameElement)
    local state = { dragging = false, dragInput = nil, dragStart = Vector2.new(), startPos = UDim2.new() }
    ...
    titleElement.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseMovement then state.dragInput = input end
    end)
...
ConfigCycleButton.MouseButton1Click:Connect(function() CycleConfigFrame.Visible = true end)
CloseCycleConfigButton.MouseButton1Click:Connect(function() CycleConfigFrame.Visible = false end)
ConfigBossesButton.MouseButton1Click:Connect(function() 
    populateBossConfigUI()
    BossConfigFrame.Visible = true 
end)
CloseBossConfigButton.MouseButton1Click:Connect(function() BossConfigFrame.Visible = false; saveBossConfig() end)
...
MinimizeButton.MouseButton1Click:Connect(function()
    ...
    if not isMinimized then
        if TowerTabButton.BackgroundColor3 == Color3.fromRGB(45, 45, 55) then TowerFrame.Visible = true else TowerFrame.Visible = false end
        if BossTabButton.BackgroundColor3 == Color3.fromRGB(45, 45, 55) then BossFrame.Visible = true else BossFrame.Visible = false end
        if MoonTabButton.BackgroundColor3 == Color3.fromRGB(45, 45, 55) then MoonFrame.Visible = true else MoonFrame.Visible = false end
    else
...
animationEvent.OnClientEvent:Connect(function()
    if isBattling then
        ...
        if currentBattleType == "tower" then TurnsLabel.Text = "Turns: " .. animationTurnCount end
    end
end)

rewardEvent.OnClientEvent:Connect(function(rewards)
    if not isBattling or (not isRunning and not isBossFarming) then return end
                ...
                for _, diff in ipairs(bossDifficulties) do
                    ...
                    if diffConfig.enabled then
                        if os.time() >= diffConfig.cooldownEnd then
                            ...
                            coroutine.wrap(monitorBattle)()
                            while isBattling and isBossFarming do wait(1) end
                            if not isBossFarming then break end
                ...
                for diff, diffConfig in pairs(boss.difficulties) do
                    if diffConfig.cooldownEnd > os.time() then
                        table.insert(activeCooldowns, {
                            name = boss.name,
                            diff = diff,
                            endTime = diffConfig.cooldownEnd
                        })
                    end
                end
            end
...
local function getMoonTier(moonName) for _, moon in ipairs(moons) do if moon.name == moonName then return moon.tier end end return 0 end
local function getMoonDisplay(moonName) for _, moon in ipairs(moons) do if moon.name == moonName then return moon.display end end return moonName end
dispatchEvent.OnClientEvent:Connect(function(data) if data and type(data) == "table" then for _, event in ipairs(data) do if event.name == "moonCycleChanged" and event.arguments and event.arguments[1] then local newMoon = event.arguments[1] if newMoon == "none" then currentMoon = "None" CurrentMoonLabel.Text = "Current Moon: None" moonTimeRemaining = 0 if moonLooping and not moonRolling then wait(2) MoonStatusLabel.Text = "Status: Moon expired, rerolling..." coroutine.wrap(rollMoons)() end else currentMoon = newMoon CurrentMoonLabel.Text = "Current Moon: " .. getMoonDisplay(currentMoon) if event.arguments[2] then moonTimeRemaining = 180 end end end end end end)
coroutine.wrap(function() while true do wait(1) if moonTimeRemaining > 0 then moonTimeRemaining = moonTimeRemaining - 1 local minutes = math.floor(moonTimeRemaining / 60) local seconds = moonTimeRemaining % 60 MoonTimerLabel.Text = string.format("Time Remaining: %dm %ds", minutes, seconds) else MoonTimerLabel.Text = "Time Remaining: 0s" end end end)()
